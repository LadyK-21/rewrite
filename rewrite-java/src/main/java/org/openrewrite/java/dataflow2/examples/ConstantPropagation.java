...
public class ConstantPropagation extends ValueAnalysis<ConstantPropagationValue> {
    ...
    {
        ProgramState<ConstantPropagationValue> state = inputState2(programPoint, new TraversalControl<>());
        ConstantPropagationValue result = state.get(v);
        return result;
    }

    @Override
    public ProgramState join(List<ProgramState<ConstantPropagationValue>> outs) {
...
//        }
    ...
    public ProgramState<ConstantPropagationValue> transferBinary(Cursor c, ProgramState<ConstantPropagationValue> inputState, TraversalControl<ProgramState<ConstantPropagationValue>> t) {
        ...
        if(cp0.getUnderstanding() == ConstantPropagationValue.Understanding.KNOWN && cp1.getUnderstanding() == ConstantPropagationValue.Understanding.KNOWN) {
            if(binary.getOperator() == J.Binary.Type.Addition) {
                if(cp0.getValue() instanceof Integer && cp1.getValue() instanceof Integer) {
                    ...
                    return inputState.push(new ConstantPropagationValue(Understanding.KNOWN, result));
        ...
        } else if(cp0.getUnderstanding() == ConstantPropagationValue.Understanding.UNKNOWN || cp1.getUnderstanding() == ConstantPropagationValue.Understanding.UNKNOWN) {
...
//        }
    ...
    public ProgramState<ConstantPropagationValue> transferLiteral(Cursor c, ProgramState<ConstantPropagationValue> inputState, TraversalControl<ProgramState<ConstantPropagationValue>> t) {
        ...
        if (pp.getType() == JavaType.Primitive.String) {
            ...
            return inputState.push(new ConstantPropagationValue(Understanding.KNOWN, value));
